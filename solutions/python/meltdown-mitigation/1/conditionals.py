...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    return (
        "green" if efficiency >= 80 
        else "orange" if efficiency >= 60 
        else "red" if efficiency >= 30 
        else "black"
    )
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    return (
        "LOW" if status < 0.9 * threshold
        else "NORMAL" if 0.9 * threshold <= status <= 1.1 * threshold
        else "DANGER"
     )
